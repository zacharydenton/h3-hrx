...
def check(name, tmp, tokens, width, x_expected, args, cfg):
    ...
    hs = tmp / f"{name}_{width}.hsaco"
    lanes = lanes_for(width)
    compile_kernel(ROOT / f"kernels/prepare_{name}_i4.loom", sym, {f"{ns}.width": width, f"{ns}.lanes": lanes, **cfg}, hs)
    ...
    want_q, want_s = R.quant_int4_rows(xr)
    got = unpack(q).astype(np.float32); wq = want_q.numpy()
    ...
    print(f"  {'PASS' if ok else 'FAIL'} prepare_{name}: tokens={tokens} width={width} lanes={lanes}  {t['per_launch_us'] / 1e3:.3f} ms  "
