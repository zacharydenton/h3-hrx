...
import os
WAVES = int(os.environ.get("ATTN_WAVES", "8")); STEM = "attention_i4qk_mha8_lds_f16_wmma" if WAVES == 8 else "attention_i4qk_mha_lds_f16_wmma"; NS, SYM = "h3." + STEM, "h3_" + STEM
...
def run(tmp, tokens, heads=HEADS):
    ...
    hs = tmp / f"{STEM}.hsaco"
    compile_kernel(ROOT / "kernels" / f"{STEM}.loom", SYM, {f"{NS}.q_stride": heads * D, f"{NS}.kv_stride": heads * D, f"{NS}.tokens": tokens, f"{NS}.token_capacity": cap, f"{NS}.scale": 1.0, f"{NS}.out_stride": heads * D}, hs)
