"""prepare_qk_i8 against a torch replica: per (token, head) mean-subtract, Sylvester H_128 (unnormalised),
...
def replica(x, mean, extra):
    """x [T][HEADS*D] f32, mean [HEADS*D] -> codes [T][HEADS*32] i32, scales [T][HEADS], dequantised rotated [T][HEADS][D]."""
    T = x.shape[0]
    ...
    q = (y / (amax / 127)).round().clamp(-127, 127)
    codes = (q.long() & 255).reshape(T, HEADS, 32, 4)
    words = torch.zeros(T, HEADS, 32, dtype=torch.int64)
    for j in range(4): words |= codes[..., j] << (8 * ((3 - j) if os.environ.get("I8_ORDER") == "rev" else j))
    words = torch.where(words >= 2 ** 31, words - 2 ** 32, words).to(torch.int32).reshape(T, HEADS * 32)
    return words, (amax[..., 0] / 127 * extra).float(), q * (amax / 127)
...
def main():
    ...
    print(f"  {'PASS' if differ < 2e-3 else 'FAIL'} prepare_qk_i8 codes: {differ * 100:.4f}% words differ (rounding ties)  {t['per_launch_us'] / 1e3:.3f} ms")
    ok = report("prepare_qk_i8 scales", scales, want_scales.numpy(), atol=1e-7, rtol=1e-4) and differ < 2e-3
    ...
    err = ((approx - exact).norm() / exact.norm()).item(); print(f"  int8 (rotated) q.k vs exact: rel err {err:.4f} (random gaussian operands)")
