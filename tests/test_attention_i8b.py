"""attention_i8qkb_mha8 (int8 QK^T, f16 PV) against a torch replica of the same quantised operands, and
against exact f16 attention; timing against the f16 kernel.
    python3 tests/test_attention_i8b.py [tokens ...]"""
import math, sys
...
import os
WAVES = int(os.environ.get("ATTN_WAVES", "8")); STEM = os.environ.get("ATTN_STEM", "attention_i8qkb_mha8_lds_f16_wmma" if WAVES == 8 else "attention_i8qkb_mha_lds_f16_wmma"); NS, SYM = "h3." + STEM, "h3_" + STEM
...
def run(tmp, tokens, heads=HEADS):
    ...
    vb = v.float().to(torch.bfloat16); v = vb.float().half()   # bf16 V: the kernel takes bf16 bits, the references the rounded values
    vT = np.ascontiguousarray(pad(vb.view(torch.int16).numpy().view(np.float16).reshape(tokens, -1), heads * D, np.float16).T)
    hs = tmp / f"{STEM}.hsaco"
    ...
    (out,), t = launch(hs, SYM, ((tokens + 16 * WAVES - 1) // (16 * WAVES), heads, 1), (32 * WAVES, 1, 1),
                       [("i32", tokens), ("i32", heads), ("in_i32", pad(qc.numpy(), heads * 32, np.int32)), ("in", pad(qs.numpy(), heads, np.float32)),
                        ("in_i32", pad(kc.numpy(), heads * 32, np.int32)), ("in", pad(ks.numpy(), heads, np.float32)), ("in_f16", vT),
