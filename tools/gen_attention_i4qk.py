...
""")
# --- the key scale: ATTN_KS=carry (default) loads the next tile's scale at the end of the iteration and carries it
# (1.036x at 15427 rows, 1.068x at 37743 on the plain kernels; the skip twins spill 60 bytes with it and run at 0.39x);
# ATTN_KS=early loads this tile's scale before the QK^T chain so its latency hides behind the MMAs, nothing carried;
# ATTN_KS=late is the original placement after the chain (ablation: 5% of the loop).
KS_MODE = os.environ.get("ATTN_KS", "carry")
if KS_MODE == "carry":
    sub("  %final_max, %final_sum, ", "  %ks_key0 = index.assume %lane_column [lt(%lane_column, %padded_tokens)] : index\n  %ks_first = view.load %ks_view[%ks_key0, %kv_head] : view<[%padded_tokens]x[%kv_head_limit]xf32> -> f32\n  %ks_last = index.sub %padded_tokens, %c1 : index\n  %final_max, %final_sum, ")
    _hdr = re.search(r"  (%final_max, %final_sum, .*?) = scf\.for %key_tile = \[%c0 to %key_tile_count step %c1\]\((.*?)\) -> \((.*?)\) \{\n", K)
    assert _hdr, "loop header"
    K = K.replace(_hdr.group(0), f"  {_hdr.group(1)}, %ks_end = scf.for %key_tile = [%c0 to %key_tile_count step %c1]({_hdr.group(2)}, %ks_carry = %ks_first : f32) -> ({_hdr.group(3)}, f32) {{\n")
    sub("    %ks_col = view.load %ks_view[%ks_key, %kv_head] : view<[%padded_tokens]x[%kv_head_limit]xf32> -> f32\n    %ks_vec = vector.splat %ks_col : vector<8xf32>\n", "    %ks_vec = vector.splat %ks_carry : vector<8xf32>\n")
    sub("    scf.yield %next_max, %next_sum, %next0, %next1, %next2, %next3, %next4, %next5, %next6, %next7 : " + ", ".join(["vector<8xf32>"] * 10) + "\n",
        "    %ks_key_n0 = index.add %local_key, %c16 : index\n    %ks_key_n1 = index.min %ks_key_n0, %ks_last : index\n    %ks_key_n = index.assume %ks_key_n1 [lt(%ks_key_n1, %padded_tokens)] : index\n    %ks_next = view.load %ks_view[%ks_key_n, %kv_head] : view<[%padded_tokens]x[%kv_head_limit]xf32> -> f32\n"
        "    scf.yield %next_max, %next_sum, %next0, %next1, %next2, %next3, %next4, %next5, %next6, %next7, %ks_next : " + ", ".join(["vector<8xf32>"] * 10) + ", f32\n")
elif KS_MODE == "early":
    sub("    %ks_key = index.assume %local_key_safe [lt(%local_key_safe, %padded_tokens)] : index\n    %ks_col = view.load %ks_view[%ks_key, %kv_head] : view<[%padded_tokens]x[%kv_head_limit]xf32> -> f32\n", "")
    sub("    %k_data0 = vector.load %k_tile[%lane_column, %c0] : view<16x20xi32> -> vector<2xi32>\n", "    %ks_key = index.assume %local_key_safe [lt(%local_key_safe, %padded_tokens)] : index\n    %ks_col = view.load %ks_view[%ks_key, %kv_head] : view<[%padded_tokens]x[%kv_head_limit]xf32> -> f32\n    %k_data0 = vector.load %k_tile[%lane_column, %c0] : view<16x20xi32> -> vector<2xi32>\n")
...
""" + K[body_start:]
    ...
    body = body.replace("    kernel.barrier<workgroup> scope(workgroup) ordering(acq_rel)\n    %ks_key_n0 = ", "    %ks_key_n0 = ")   # the carried key scale's lines sit before the yield
    assert body.count("scope(workgroup)") == 1, "the double-buffered loop keeps one barrier per tile"
...
def uncarry(K: str) -> str:
    ...
    assert hdr, "carried loop header"
    K = K.replace(hdr.group(0), f"  {hdr.group(1)} = scf.for %key_tile = [%c0 to %key_tile_count step %c1]({hdr.group(2)}) -> ({hdr.group(3)}) {{\n")
    K = K.replace("    %ks_vec = vector.splat %ks_carry : vector<8xf32>\n", "    %ks_col = view.load %ks_view[%ks_key, %kv_head] : view<[%padded_tokens]x[%kv_head_limit]xf32> -> f32\n    %ks_vec = vector.splat %ks_col : vector<8xf32>\n")
    K = K.replace("    %ks_key_n0 = index.add %local_key, %c16 : index\n    %ks_key_n1 = index.min %ks_key_n0, %ks_last : index\n    %ks_key_n = index.assume %ks_key_n1 [lt(%ks_key_n1, %padded_tokens)] : index\n    %ks_next = view.load %ks_view[%ks_key_n, %kv_head] : view<[%padded_tokens]x[%kv_head_limit]xf32> -> f32\n", "")
    K = K.replace(", %ks_next : " + ", ".join(["vector<8xf32>"] * 10) + ", f32\n", " : " + ", ".join(["vector<8xf32>"] * 10) + "\n")
    assert "%ks_carry" not in K and "%ks_next" not in K and "%ks_first" not in K, "uncarry incomplete"
    return K
