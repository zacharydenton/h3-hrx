"""The C host against ComfyUI from ComfyUI's own noise (tools/comfy_clip.py --dump-steps [--dump-blocks ...]):
  ...
  blocks:     one evaluation, the refined text rows and the residual stream after chosen blocks, cosine per row segment
    python tools/compare_comfy.py --case t2va --mode blocks --truth build/comfy_t2va_blocks --attn f16
...
ap.add_argument("--truth", required=True, help="tools/comfy_clip.py --out directory"); ap.add_argument("--dit", default=None, help="the DiT checkpoint (default: the fl2va file under $H3_MODELS)"); ap.add_argument("--attn", choices=["i4", "i8", "f16"], default="i8")
...
t0 = time.time(); v, au = pipe.denoise(ids, p, noise_video=nv, noise_audio=np.ascontiguousarray(na.transpose(1, 0, 2)), keyframes=kfs); print(f"C denoised in {time.time() - t0:.1f} s ({Path(dit).stem}, {a.attn} attention)", flush=True)
