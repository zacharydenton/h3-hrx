...
mod atoms { rustler::atoms! { ok, h3_result } }
enum Job { Ping(LocalPid, u64), Encode(LocalPid, u64, Vec<f32>, usize) }
struct Model { jobs: SyncSender<Job> }
...
fn open(audio_checkpoint: Option<String>) -> NifResult<ResourceArc<Model>> {
    let config = h3::Config { dit: None, te: None, video_vae: None,
        audio_vae: audio_checkpoint.map(Into::into), ..Default::default() };
    // The application owns and keeps the configured checkpoint immutable.
    let mut session = unsafe { h3::Session::new(config) }.map_err(|e| rustler::Error::Term(Box::new(e.to_string())))?;
    let (jobs, receiver) = mpsc::sync_channel(2);
    std::thread::Builder::new().name("h3-model".into()).spawn(move || {
        while let Ok(job) = receiver.recv() {
            let (pid, id, result) = match job {
                Job::Ping(pid, id) => (pid, id, Ok((Vec::new(), 0usize))),
                Job::Encode(pid, id, samples, frames) =>
                    (pid, id, session.encode_audio(&samples, frames).map_err(|e| e.to_string())),
            };
            let mut env = OwnedEnv::new();
            // A dead caller does not invalidate model state or keep a NIF borrow.
            let _ = env.send_and_clear(&pid, |env| (atoms::h3_result(), id, result).encode(env));
        }
...
fn ping(env: Env<'_>, model: ResourceArc<Model>, id: u64) -> NifResult<Atom> {
    model.jobs.try_send(Job::Ping(env.pid(), id)).map_err(|e| rustler::Error::Term(Box::new(e.to_string())))?;
    Ok(atoms::ok())
}
#[rustler::nif(schedule = "DirtyCpu")]
fn encode_audio(env: Env<'_>, model: ResourceArc<Model>, samples: Binary<'_>, frames: usize, id: u64) -> NifResult<Atom> {
    if !samples.len().is_multiple_of(4) { return Err(rustler::Error::BadArg); }
    let samples = samples.as_slice().chunks_exact(4).map(|b| f32::from_le_bytes(b.try_into().unwrap())).collect();
    model.jobs.try_send(Job::Encode(env.pid(), id, samples, frames)).map_err(|e| rustler::Error::Term(Box::new(e.to_string())))?;
    Ok(atoms::ok())
}
