...
step "generated kernels match their generators" bash -c '
  ...
  cp -r kernels "$tmpdir/kernels" && cp -r tools "$tmpdir/tools" && mkdir -p "$tmpdir/experiments" && cp -r scripts "$tmpdir/scripts" && cd "$tmpdir" &&
  python3 tools/gen_prepare.py >/dev/null && python3 tools/gen_attention_lds.py >/dev/null && ATTN_WAVES=8 python3 tools/gen_attention_lds.py >/dev/null && ATTN_GQA=8 ATTN_WAVES=8 ATTN_CAUSAL=1 python3 tools/gen_attention_lds.py >/dev/null && python3 tools/gen_gemm.py >/dev/null && python3 tools/gen_rope.py >/dev/null && sh scripts/gen_attention_i4.sh >/dev/null &&
...
step "CPU host regressions" bash scripts/test_host.sh
step "small kernel regressions" bash -c 'env -u LD_LIBRARY_PATH python3 tests/test_kernel_regressions.py'
...
step "C tokenizer vs transformers" bash -c 'source ~/code/krea2-loom/.venv/bin/activate && env -u LD_LIBRARY_PATH python3 tests/test_tokenizer.py'
if [ "${1:-}" != "--quick" ]; then
  step "tiled decoder parity (decoder weights only)" bash -c 'source ~/code/krea2-loom/.venv/bin/activate && env -u LD_LIBRARY_PATH python3 tests/test_decoder_tiles.py'
