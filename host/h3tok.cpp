...
extern "C" const char h3tok_embedded_json[], h3tok_embedded_json_end[];   // assets/tokenizer.json (host/tokenizer_blob.S)

namespace {
using h3json::Json; using h3json::Parser;
...
struct Tok {
    ...
    // path: a tokenizer.json to read; empty: the embedded assets/tokenizer.json (Qwen's, as ComfyUI bundles it)
    explicit Tok(const std::string &path) {
        std::string src;
        if (path.empty()) src.assign(h3tok_embedded_json, size_t(h3tok_embedded_json_end - h3tok_embedded_json));
        else { std::ifstream f(path); if (!f) throw std::runtime_error("cannot read " + path); std::stringstream ss; ss << f.rdbuf(); src = ss.str(); }
        Json root = h3json::parse(src, path.empty() ? "the embedded tokenizer.json" : path.c_str());
...
extern "C" h3tok *h3tok_create(const char *path, char *error, size_t cap) {
    ...
    try {
        const char *env = std::getenv("H3_TOKENIZER");   // H3_TOKENIZER=<file> replaces the embedded tokenizer when no path is given
        return new h3tok(path ? std::string(path) : (env && *env ? std::string(env) : std::string()));
    }
