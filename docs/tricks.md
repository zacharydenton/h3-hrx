...
| Qwen3-VL-32B in Loom, int8 | `host/h3pipe.cpp` (the text encoder stack), `kernels/` | a 32B vision-language encoder as a C library: embeddings, prefill, the vision tower with DeepStack |
...
| the video VAE (causal 3-D conv encoder and decoder) and the audio VAE in Loom | `host/h3pipe.cpp`, `kernels/conv*`, `kernels/*snake*` | standalone codecs: the decoder tiles as ComfyUI does, 52 dB frame PSNR at int8 |
