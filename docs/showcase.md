...
The alien-fjord, tidal-sky, whale, and desert scenes use text only. Glass Leviathan
and the iceberg animate generated first frames.

Each clip contains 124 frames at 24 fps (5.17 seconds), with 20 evaluations of
`res_multistep`, with no frame interpolation or upscaling. Glass Leviathan,
alien-fjord, and tidal-sky are native 1344×768; the older whale, desert, and iceberg
clips are 864×480.
...
sound. The supplied PNG is the untouched conditioning image. The complete render
took **39 min 58 s** on Strix Halo; [timing and memory](benchmarks/20260914/glass-leviathan.json)
are recorded separately from the text-to-video comparison.
